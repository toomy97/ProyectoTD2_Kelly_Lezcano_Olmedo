...
// SCK				PB10				SPI1_SCK	Serial Clock for SPI
// MOSI				PC3 				SPI1_MOSI	Master In Slave Out for SPI
// MISO				PC2					SPI1_MISO	Master Out Slave In for SPI
// IRQ				-					Irq
// GND				GND					Ground
...
//	HAL_SPI_Transmit(&hspi2, (uint8_t*)&writeCommand, 1, 10);

	return readValue[0];

	//while (SPI1->SR & SPI_SR_BSY);								// STM32F030 - Ð¶Ð´ÐµÐ¼ ÐºÐ¾Ð½ÐµÑ† Ð¿ÐµÑ€ÐµÐ´Ð°Ñ‡Ð¸
	//while (SPI1->SR & SPI_I2S_FLAG_BSY);					// STM32F103 - Ð¶Ð´ÐµÐ¼ ÐºÐ¾Ð½ÐµÑ† Ð¿ÐµÑ€ÐµÐ´Ð°Ñ‡Ð¸

	//while (!(SPI1->SR & SPI_SR_TXE));     				// ÑƒÐ±ÐµÐ´Ð¸Ñ‚ÑŒÑÑ, Ñ‡Ñ‚Ð¾ Ð¿Ñ€ÐµÐ´Ñ‹Ð´ÑƒÑ‰Ð°Ñ Ð¿ÐµÑ€ÐµÐ´Ð°Ñ‡Ð° Ð·Ð°Ð²ÐµÑ€ÑˆÐµÐ½Ð° (STM32F103)
	//SPI1->DR = data;															// Ð²Ñ‹Ð²Ð¾Ð´ Ð² SPI1
	//while (!(SPI1->SR & SPI_SR_RXNE));     				// Ð¶Ð´ÐµÐ¼ Ð¾ÐºÐ¾Ð½Ñ‡Ð°Ð½Ð¸Ñ Ð¾Ð±Ð¼ÐµÐ½Ð° (STM32F103)
	//for (uint8_t i=0; i<50; i++) {};
	//data = SPI1->DR;															// Ñ‡Ð¸Ñ‚Ð°ÐµÐ¼ Ð¿Ñ€Ð¸Ð½ÑÑ‚Ñ‹Ðµ Ð´Ð°Ð½Ð½Ñ‹Ðµ
	//return data;
}
...
void MFRC522_Init(void)
{
	...
	MFRC522_Reset();
	MFRC522_WriteRegister(MFRC522_REG_T_MODE, 0x80);//,0x8D
	MFRC522_WriteRegister(MFRC522_REG_T_PRESCALER, 0xA9);//,0x3E
	MFRC522_WriteRegister(MFRC522_REG_T_RELOAD_L, 0xE8);//,30
	MFRC522_WriteRegister(MFRC522_REG_T_RELOAD_H, 0x03);//,0
	MFRC522_WriteRegister(MFRC522_REG_RF_CFG, 0x70);	//,0x70			// 48dB gain
